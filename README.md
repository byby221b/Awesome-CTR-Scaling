...
| **ReST: From Language to Behavior: Scaling Sequence Transformers for Industrial Recommendation Ranking with Rec-Native Designs** | ByteDance | arXiv | 2026 | `Architecture` `Attention` `Sequence Modeling` `Serving` | [[Paper]](https://arxiv.org/abs/2609.01240) | Rec-native Transformer scaling: dual-gated attention + RoPE temporal encoding for noisy behavior sequences; heavy encoder + lightweight cross decoder with shared-prefix serving for compute-once decode-many ranking; AUC +1.31%, revenue +11.93% in ads A/B test |
